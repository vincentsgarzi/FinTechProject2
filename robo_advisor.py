...
########################################
# Streamlit Page Configuration & Logging
########################################
...
#########################
with tracer.trace("sidebar.setup", service="robo_advisor"):
    with st.sidebar:
        ...
        selected_tickers = st.multiselect(
            ':blue[Select the stock tickers you wish to incorporate into your portfolio.]',
            nyse_tickers
        )
        ...
        for ticker in selected_tickers:
            ...
            weight = st.slider(
                ...
                key=ticker_symbol, min_value=.0,
                max_value=1.0, value=0.0, step=.05
            )
...
with tab1:
    ...
    # Description
    st.write(
        "Our investment portfolio finanical application enables you to build and manage "
        "your investment portfolio. It is designed to help you make informed investment "
    ...
    with st.expander("Portfolio Overview"):
        st.write(
            "Our Portfolio Dashboard tab provides you with real-time information about "
            "your investment portfolio. You can view the current status of your investments "
            "and get access to the latest news and analysis about the stocks in your portfolio. "
            "This feature helps you stay up-to-date on the latest market trends and make informed "
            "decisions about your investment strategy."
        )
        st.write(
            "This tab also offers a detailed breakdown of your portfolio, which provides you "
            "with a comprehensive overview of your investment allocations. You can easily see "
            "how much you've invested in each stock, sector, and asset class, allowing you to make "
            "any necessary adjustments to your portfolio based on your financial goals and risk tolerance."
        )

    # What If Section
    with st.expander("What-If"):
        st.write(
            "The 'What If' tab is a web tool that helps you see how your portfolio would "
            "have performed if you had invested it over different time periods in the past."
        )
        st.write(
            'It uses sophisticated algorithms and data analysis techniques to give you a clear picture '
            'of the potential value of your portfolio. By theorizing how your investments would have '
            'performed over time, you can gain valuable insights into their historical performance and '
    ...
    with st.expander("Robo Advisor"):
        st.write(
            "Our machine learning algorithms and robo advisor feature provide you with a powerful tool "
            "to automatically trade the stocks in your portfolio. Our algorithms use the latest data analysis "
            "techniques to identify investment opportunities and help you make profitable trades. This feature "
            "allows you to take advantage of the latest market trends without spending countless hours analyzing "
            "market data."
        )
    ...
    with st.expander("User-Friendly Interface"):
        st.write(
            "Our platform is designed to be easy to use, even if you have no previous investment experience."
        )
        st.write(
            "With that said, we are committed to helping you achieve your financial goals. Whether you're looking "
            "to build a long-term investment portfolio or trade stocks for short-term gains, our platform provides "
            "you with the tools and resources you need to succeed. Enjoy!"
        )
...
with tab2:
    ...
    st.write(
        "Below, you'll find an overview of your selected investment portfolio, including weightings and "
        "historical close data. You'll also see pertinent news related to the portfolio you've constructed."
    )
    st.title('')

    try:
        with tracer.trace("gather_data", service="robo_advisor"):
            logger.info("calling the gatherData function to get the stock ticker data")
            market_data = gatherData(
                tickers=ticker_keys,
        ...
        for ticker in ticker_keys:
            ticker_df = market_data[market_data['symbol'] == ticker].drop('symbol', axis=1)
            tickers_dfs.append(ticker_df)

        with tracer.trace("concat_dataframes", service="robo_advisor"):
        ...
        logger.error(f"Must build your portfolio to proceed. Error: {e}")
    ...
    if total_weight != 1:
        ...
        with tracer.trace("portfolio_breakdown", service="robo_advisor"):
        ...
        with col1:
            ...
            with chart1:
                with tracer.trace("plot_close_prices_5y", service="robo_advisor"):
                    close_plot = plot_close_prices(
                        ticker_keys=ticker_keys,
                        concat_market_data=concat_market_data,
                        selected_period=num_days[0]
                    )
                    st.plotly_chart(close_plot, use_container_width=True, key="chart_4")

            with chart2:
                with tracer.trace("plot_close_prices_1y", service="robo_advisor"):
                    close_plot = plot_close_prices(
                        ticker_keys=ticker_keys,
                        concat_market_data=concat_market_data,
                        selected_period=num_days[1]
                    )
                    st.plotly_chart(close_plot, use_container_width=True, key="chart_5")

            with chart3:
                with tracer.trace("plot_close_prices_6m", service="robo_advisor"):
                    close_plot = plot_close_prices(
                        ticker_keys=ticker_keys,
                        concat_market_data=concat_market_data,
                        selected_period=num_days[2]
                    )
                    st.plotly_chart(close_plot, use_container_width=True, key="chart_6")

            with chart4:
                with tracer.trace("plot_close_prices_3m", service="robo_advisor"):
                    close_plot = plot_close_prices(
                        ticker_keys=ticker_keys,
                        concat_market_data=concat_market_data,
                        selected_period=num_days[3]
                    )
                    st.plotly_chart(close_plot, use_container_width=True, key="chart_7")

            with chart5:
                with tracer.trace("plot_close_prices_7d", service="robo_advisor"):
                    close_plot = plot_close_prices(
                        ticker_keys=ticker_keys,
                        concat_market_data=concat_market_data,
                        selected_period=num_days[4]
                    )
                    st.plotly_chart(close_plot, use_container_width=True, key="chart_8")

        with col2:
            st.subheader('Current Close Prices')
            with tracer.trace("get_closing_prices", service="robo_advisor"):
                ...
                current_close_df = get_closing_prices(
                    tickers=ticker_keys,
            ...
            with tracer.trace("get_news_headlines", service="robo_advisor"):
                logger.info("calling the 'get_news_headlines' function")
                news = get_news_headlines(
                    tickers=ticker_keys,
                    api_key=alpaca_api_key,
                    secret_key=alpaca_secret_key
                )
            # You can display the news if desired. For now, you’re just retrieving it.
...
with tab3:
    st.title('What-If')
    st.write(
        'Discover the potential value of your investment portfolio with our advanced analysis tool. '
        'It takes the portfolio you built and visualizes how your investments **:blue[could have]** grown '
        'over various time periods. It provides valuable insights into the performance of your investments '
        'and helps you make informed decisions about your financial future.'
    )
    ...
    if total_weight != 1.0:
        ...
        logger.info("retrieving an integer from a string for the time period")
        ...
        with tracer.trace("portfolio_returns", service="robo_advisor"):
            logger.info("calling the portfolio_returns function to get probable returns and the plot")
            return_num, return_plot = portfolio_returns(
                concat_market_data,
                ticker_keys,
                ticker_weights,
                investment_amount,
                selected_period=number_days
            )

        co1, co2 = st.columns([2.5, .9], gap='large')
        with co1:
            st.subheader(
                f"If you would have invested :blue[**${investment_amount}**]  {selected_period.lower()} ago, "
                f"your portfolio would have an estimated value of :blue[**${return_num:.2f}**] today."
            )
...
with tab4:
    ...
    if len(ticker_keys) != 0:
        with tracer.trace("robo_advisor_data", service="robo_advisor"):
            ...
            logger.info("calling the 'createSignals' function to return a dataframe with the buy and sell data")
        ...
        with side1:
            st.write(
                'Our robo-advisor tab uses machine learning and simple moving averages to provide you with real-time '
                'recommendations on when to buy or sell specific stocks in your portfolio. The red and green arrows '
                'indicate whether you should sell or buy a stock, respectively, based on our analysis of market trends '
                'and performance data.'
            )
            st.write(
                'You can visualize each stock in your portfolio by selecting it from the dropdown menu. Our data-driven '
                'recommendations take the guesswork out of investing and help you make informed decisions to optimize '
        ...
        with side2:
            ...
            logger.info("indicating whether the user should buy, sell, or hold for each stock in portfolio")
            index = 0
            for signal in signals_df:
                position = ''
                sig = signal.iloc[-1]['Entry/Exit']
                if sig == 0.0:
